...
public class StopwatchSample extends Sample {
	...
	public boolean equals(Object o) {
		...
		StopwatchSample sample = (StopwatchSample) o;

		if (active != sample.active) return false;
		if (counter != sample.counter) return false;
		if (last != sample.last) return false;
		if (max != sample.max) return false;
		if (maxActive != sample.maxActive) return false;
		if (maxActiveTimestamp != sample.maxActiveTimestamp) return false;
		if (maxTimestamp != sample.maxTimestamp) return false;
		if (Double.compare(sample.mean, mean) != 0) return false;
		if (min != sample.min) return false;
		if (minTimestamp != sample.minTimestamp) return false;
		if (Double.compare(sample.standardDeviation, standardDeviation) != 0) return false;
		if (total != sample.total) return false;
		if (Double.compare(sample.variance, variance) != 0) return false;
		if (Double.compare(sample.varianceN, varianceN) != 0) return false;
	...
	public int hashCode() {
		...
		result = 31 * result + (int) (temp ^ (temp >>> 32));
		temp = Double.doubleToLongBits(varianceN);
		result = 31 * result + (int) (temp ^ (temp >>> 32));
		return result;
	}
}
